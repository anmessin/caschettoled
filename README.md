...
la classe opera nel suo dominio di applicazione: controlla che mi sia collegato alla board, che gli input ricevuti siano validi e deve raise l'errore, ma alla classe non importa come quello verrà gestito.

Nel caso di applicativi diversi che gesticono la classe, una potrebbe voler comunque continuare, un'altra potrebbe invece far fermare l'applicazione, altre invece potrebbero0 voler metter in log il fatto.


- Chiedere di cambiare l'endpoint settrigerFreq in setTriggWerFreq
- Modificare le variabili per i mini e massimim in liste e cercare di riportare in funzioni le varie validazioni che si ripeteono per ogni metodo. (manca per i range e la validazione del sweep mode)